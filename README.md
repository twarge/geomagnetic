...
XcodeGen). The generated `Geomagnetic.xcodeproj` is committed, so you can open it directly;
re-run the generator after adding or removing source files:

```sh
python3 Scripts/generate-project.py      # regenerate Geomagnetic.xcodeproj
./Scripts/build-all.sh                    # regenerate + build macOS, iOS, watchOS
```

Or from Xcode: open `Geomagnetic.xcodeproj` and run the **Observatory** or
