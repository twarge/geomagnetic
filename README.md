...
Scripts/                  Project generator + build helpers
```
...
The Xcode project is generated by a small, dependency-free script (in the spirit of
XcodeGen). The generated `observatory.xcodeproj` is committed, so you can open it directly;
re-run the generator after adding or removing source files:

```sh
python3 Scripts/generate-project.py      # regenerate observatory.xcodeproj
./Scripts/build-all.sh                    # regenerate + build macOS, iOS, watchOS
```

Or from Xcode: open `observatory.xcodeproj` and run the **Observatory** or
**ObservatoryWatch** scheme.
...
xcodebuild -project observatory.xcodeproj -scheme ObservatoryWatch \
